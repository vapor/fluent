...
public protocol AnyMigration {
    ...
    static func migrationPrepare(any: DatabaseConnection) -> Future<Void>

    /// Closure for reverting the migration.
    static func migrationRevert(any: DatabaseConnection) -> Future<Void>
...
extension AnyMigration where Self: Migration {
    ...
    /// See `Migration`.
    public static func migrationPrepare(any: DatabaseConnection) -> Future<Void> {
        return prepare(on: convert(any: any))
    }

    /// See `Migration`.
    public static func migrationRevert(any: DatabaseConnection) -> Future<Void> {
    ...
    private static func convert(any: DatabaseConnection) -> Database.Connection {
