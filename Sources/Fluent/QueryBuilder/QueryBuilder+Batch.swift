extension QueryBuilder {
    ...
    public func create(_ data: [Model.Database.Query.Field: Model.Database.Query.Data]) -> Future<Void> {
        query.fluentData = data
        return run(.fluentCreate)
    }
    ...
    public func update(_ data: [Model.Database.Query.Field: Model.Database.Query.Data]) -> Future<Void> {
        query.fluentData = data
    ...
    public func update<Value>(_ key: KeyPath<Model, Value>, to value: Value) -> Future<Void> where Value: Encodable {
        query.fluentData[.keyPath(key)] = .fluentEncodable(value)
        return run(.fluentUpdate)
    }
}
