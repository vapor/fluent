...
extension QueryRepresentable {
    ...
    public func filter<T: Entity>(
        ...
        let filter = Filter(entity, .compare(field, comparison, try value.makeNode(in: nil)))
    ...
    public func filter<T: Entity>(
        ...
        let query = try makeQuery()
        let filter = Filter(T.self, .subset(field, scope, try set.map({ try $0.makeNode(in: nil) })))
        query.filters.append(filter)
        return query
    }
...
extension QueryRepresentable {
    ...
    public func raw(command: String, values: [NodeRepresentable] = []) throws -> Query<Self.T> {
        ...
        let values = try values.map { try $0.makeNode(in: nil) }
