...
extension SchemaBuilder where Model.Database: IndexSupporting {
    ...
    {
        let index = SchemaIndex<Model.Database>(fields: [field.makeQueryField()], isUnique: isUnique)
    ...
    public func addIndex<T, U>(to fieldA: KeyPath<Model, T>, _ fieldB: KeyPath<Model, U>, isUnique: Bool = false)
        where T: KeyStringDecodable, U: KeyStringDecodable
    {
        let index = SchemaIndex<Model.Database>(fields: [fieldA.makeQueryField(), fieldB.makeQueryField()], isUnique: isUnique)
        schema.addIndexes.append(index)
    }
    ...
    public func removeIndex<T>(from field: KeyPath<Model, T>)
        where T: KeyStringDecodable
    {
        let index = SchemaIndex<Model.Database>(fields: [field.makeQueryField()], isUnique: false)
    ...
    public func removeIndex<T, U>(from fieldA: KeyPath<Model, T>, _ fieldB: KeyPath<Model, U>)
        where T: KeyStringDecodable, U: KeyStringDecodable
    {
        let index = SchemaIndex<Model.Database>(fields: [fieldA.makeQueryField(), fieldB.makeQueryField()], isUnique: false)
        schema.removeIndexes.append(index)
    }
}
