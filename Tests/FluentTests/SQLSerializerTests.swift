...
class SQLSerializerTests: XCTestCase {
    static let allTests = [
        ...
        ("testGroupByMultiple", testGroupByMultiple),
        ("testGroupByCount", testGroupByCount)
    ]

    func testBasicSelect() {
        let sql = SQL.select(table: "users", filters: [], joins: [], groups: [], orders: [], limit: nil)
    ...
    func testRegularSelect() {
        ...
        let sql = SQL.select(table: "users", filters: [filter], joins: [], groups: [], orders: [], limit: Limit(count: 5))
        let (statement, values) = serialize(sql)
    ...
    func testOffsetSelect() {
        let filter = Filter(User.self, .compare("age", .greaterThanOrEquals, 21))
        let sql = SQL.select(table: "users", filters: [filter], joins: [], groups: [], orders: [], limit: Limit(count: 5, offset: 15))
    ...
    func testFilterCompareSelect() {
        ...
        let select = SQL.select(table: "friends", filters: [filter], joins: [], groups: [], orders: [], limit: nil)
    ...
    func testBasicCount() {
        let sql = SQL.count(table: "users", filters: [], joins: [], groups: [])
        let (statement, values) = serialize(sql)
    ...
    func testRegularCount() {
        let filter = Filter(User.self, .compare("age", .greaterThanOrEquals, 21))
        let sql = SQL.count(table: "users", filters: [filter], joins: [], groups: [])
        let (statement, values) = serialize(sql)
    ...
    func testFilterCompareCount() {
        ...
        let select = SQL.count(table: "friends", filters: [filter], joins: [], groups: [])
        let (statement, values) = serialize(select)
    ...
    func testFilterGroup() throws {
        ...
        let select = SQL.select(table: "users", filters: [one, group, four], joins: [], groups: [], orders: [], limit: nil)
        let (statement, values) = serialize(select)
    ...
    func testSort() throws {
        ...
        let select = SQL.select(table: "users", filters: [adult], joins: [], groups: [], orders: [name], limit: nil)
        let (statement, values) = serialize(select)
    ...
    func testSortMultiple() throws {
        ...
        let select = SQL.select(table: "users", filters: [adult], joins: [], groups: [], orders: [name, email], limit: nil)
        let (statement, values) = serialize(select)
    ...
    func testGroupBy() throws {
        ...
        let group = GroupBy(User.self, "name")
        
        let select = SQL.select(table: "users", filters: [], joins: [], groups: [group], orders: [], limit: nil)
        let (statement, values) = serialize(select)
        
        XCTAssertEqual(statement, "SELECT `users`.* FROM `users` GROUP BY `users`.`name`")
        XCTAssertEqual(values.count, 0)
    }
    
    func testGroupByMultiple() throws {
        let groupName = GroupBy(User.self, "name")
        let groupEmail = GroupBy(User.self, "email")
        let age = Sort(User.self, "age", .descending)
        
        let select = SQL.select(table: "users", filters: [], joins: [], groups: [groupName, groupEmail], orders: [age], limit: nil)
        let (statement, values) = serialize(select)
        
        XCTAssertEqual(statement, "SELECT `users`.* FROM `users` GROUP BY `users`.`name`, `users`.`email` ORDER BY `users`.`age` DESC")
        XCTAssertEqual(values.count, 0)
    }
    ...
    func testGroupByCount() throws {
        ...
        let select = SQL.count(table: "users", filters: [], joins: [], groups: [group])
        let (statement, values) = serialize(select)
        
        XCTAssertEqual(statement, "SELECT COUNT(*) as _fluent_count FROM `users` GROUP BY `users`.`name`")
        XCTAssertEqual(values.count, 0)
    }
}
